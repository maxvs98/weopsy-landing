...
export default async function ArchivedDocumentsPage({ params }: { params: Promise<{ locale: LocaleCode }> }) {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: LocaleCode }> }): Promise<Metadata> {
  ...
  return createMetadata({ locale: 'en', title: t('metadata.title'), canonical: '/archived-docs' });
} 