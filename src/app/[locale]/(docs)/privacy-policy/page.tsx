...
export default async function PrivacyPolicyPage({ params }: { params: Promise<{ locale: LocaleCode }> }) {
...
export async function generateMetadata({ params }: { params: Promise<{ locale: LocaleCode }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: 'en', namespace: 'pages.privacyPolicy' });
